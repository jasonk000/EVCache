...
public interface NodeLocatorLookup<V> {
    ...
    static class FastAVLTreeNodeLocatorLookup<V> implements NodeLocatorLookup<V> {
        private final Long2ObjectAVLTreeMap<V> map;

        FastAVLTreeNodeLocatorLookup(TreeMap<Long, V> map) {
            TreeMap<Long, V> clone = new TreeMap<>(map);
            coalesceMap(clone);
            this.map = new Long2ObjectAVLTreeMap<>(clone);
        }

        @Override
        public V wrappingCeilingValue(long hash) {
            if (hash > map.lastLongKey()) {
                return map.values().iterator().next();
            }

            return map.tailMap(hash).values().iterator().next();
        }
    }
    ...
    static class ArrayNodeLocatorLookup<V> implements NodeLocatorLookup<V> {
        ...
        ArrayNodeLocatorLookup(TreeMap<Long, V> _map) {
            TreeMap<Long, V> map = new TreeMap<>(_map);
            coalesceMap(map);
